        ...
        if self.log_dir is not None and not self.disable_logs:
            ...
            if self.ep_extras:
                ...
                for key in self.ep_extras[0]:
                    ...
                    extras_info[key] = value.item()
                sorted_extras_dict = dict(sorted(extras_info.items(), key=lambda item: item[1]))
                reward_dict = {}
                termination_dict = {}
                curriculum_dict = {}
                metrics_dict = {}
                other1_dict = {}
                other2_dict = {}
                for key, value in sorted_extras_dict.items():
                    if "/" in key:
                        if "_Reward" in key:
                            reward_dict[key] = value
                        elif "_Termination" in key:
                            termination_dict[key] = value
                        elif "Curriculum/" in key:
                            curriculum_dict[key] = value
                        elif "Metrics/" in key:
                            metrics_dict[key] = value
                        else:
                            other1_dict[key] = value
                        self.writer.add_scalar(key, value, it)
                        # extras_string += f"""{f"{key}:":>{pad}} {value:.4f}\n"""
                    else:
                        self.writer.add_scalar("Episode/" + key, value, it)
                        other2_dict[key] = value.item()
                ...
                if termination_dict:
                    extras_string += "\n"
                    for key, value in termination_dict.items():
                        extras_string += f"""{f"{key}:":>{pad}} {value:.4f}\n"""
                if curriculum_dict:
                    extras_string += "\n"
                    for key, value in curriculum_dict.items():
                        extras_string += f"""{f"{key}:":>{pad}} {value:.4f}\n"""
                if metrics_dict:
                    extras_string += "\n"
                ...
                if other1_dict:
                    extras_string += "\n"
                    for key, value in other1_dict.items():
                        extras_string += f"""{f"{key}:":>{pad}} {value:.4f}\n"""
                if other2_dict:
                    extras_string += "\n"
                    for key, value in other2_dict.items():
                        extras_string += f"""{f"Mean episode {key}:":>{pad}} {value:.4f}\n"""
